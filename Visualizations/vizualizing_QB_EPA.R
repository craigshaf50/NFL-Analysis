...
#---QB EPA per play from 2020-2022---
...
qb_epa_play <- pbp %>% 
  ...
  filter(plays >= 750, pass_attempts >= 150) %>% 
  mutate(pass_rate = pass_attempts / plays) %>% 
  left_join(teams_colors_logos, by = c("team" = "team_abbr"))

#scatter plot with qb_epa_play
qb_epa_play %>% 
  ...
  labs(x = "Pass Rate",
       ...
       subtitle = "Minimum of 750 plays and 150 pass attempts to be included") +
...
qb_epa_play %>% 
  ...
  labs(x = "EPA/Play",
       ...
       subtitle = "Minimum of 750 plays and 150 pass attempts") +
  theme(panel.grid.major.y = element_blank(),
        plot.title = element_text(size = 22, hjust = 0.5, face = "bold"),
        plot.subtitle = element_text(size = 16, hjust = 0.5)) 

#gt table with the same information
qb_gt <- qb_epa_play %>% 
  arrange(-epa_play) %>%
